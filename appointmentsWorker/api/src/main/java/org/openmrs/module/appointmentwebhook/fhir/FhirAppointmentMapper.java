...
public class FhirAppointmentMapper {
	...
	public static String toFhirJson(Appointment src, String serverBase) {
        ...
        // ── serviceType (from AppointmentServiceDefinition) ───────────
        AppointmentServiceDefinition service = src.getService();
        if (service != null) {
        ...
        // ── serviceCategory (from AppointmentServiceType) ─────────────
        AppointmentServiceType serviceType = src.getServiceType();
        if (serviceType != null) {
        ...
        // ── appointmentType (Walk-In vs Scheduled) ────────────────────
        AppointmentKind kind = src.getAppointmentKind();
        if (kind != null) {
        ...
        // 1) Patient
        Patient patient = src.getPatient();
        if (patient != null) {
            j.objectStart();
            ...
            if (pid != null) {
                ...
                j.key("system").value(OPENMRS_SYSTEM + "/patient-identifier/"
                        + (pid.getIdentifierType() != null
                        ? pid.getIdentifierType().getUuid() : "default"));
            ...
            j.objectEnd(); // participant
        }

        // 2) Providers (Bahmni supports multiple providers per appointment)
        Set<AppointmentProvider> providers = src.getProviders();
        if (providers != null) {
            for (AppointmentProvider ap : providers) {
                ...
                j.key("required").value("required");
                // Map provider response to FHIR participation status
        ...
        // 3) Location
        Location location = src.getLocation();
        if (location != null) {
            j.objectStart();
